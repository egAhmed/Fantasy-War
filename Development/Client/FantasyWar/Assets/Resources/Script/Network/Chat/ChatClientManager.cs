...
public class ChatClientManager : UnitySingleton<ChatClientManager>
{
    ...
    {
        //  
        IPAddress ip = IPAddress.Parse(NetworkConfig.SERVER_HOST);
        clientSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        //
        try
        {
            clientSocket.Connect(new IPEndPoint(ip, NetworkConfig.SERVER_PORT)); //binding ip and port config  
            //
        ...
        if (msgStr != null)
            {
                ...
                msg.name = NetworkConfig.UserName;
                msg.msg = msgStr;
                if (msg != null)
                {
